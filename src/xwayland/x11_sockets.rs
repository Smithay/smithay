...
/// Find a free X11 display slot and set up.
    ...
    fn grab(number: u32) -> Result<X11Lock, ()> {
        ...
        match lockfile {
            Ok(mut file) => {
                ...
                if ret.is_err() {
                    // Write to the file failed, so we abandon it.
                    ::std::mem::drop(file);
            ...
            Err(_) => {
                ...
                // If it contains the PID of a process that no longer
                // exists (so if a previous X server claimed it and did not
