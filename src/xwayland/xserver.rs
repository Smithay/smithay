 ...
 *    startup completes. We want to catch this so we can launch the WM.
 * -> we need to track if Xwayland crashes, to restart it.
 *
 * cf https://github.com/swaywm/wlroots/blob/master/xwayland/xwayland.c
 *
 * Setting up the SIGUSR1 handler is complicated in a multithreaded program, because
 ...
 * a multithreaded program, as after forking only signal-safe functions can be used.
...
impl Xwayland {
    ...
    /// - `envs` - Allows additional environment variables for the Xwayland executable to use.
...
impl XwaylandClientData {
    /// Access user_data map for an Xwayland client.
