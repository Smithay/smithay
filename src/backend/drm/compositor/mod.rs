...
pub trait AsRenderTarget<B: allocator::Buffer, R: Renderer> {
...
/// Default implementation for AsRenderTarget
#[derive(Debug)]
    ...
    R: Renderer + Bind<Dmabuf>,
{
    ...
    type Error = crate::backend::allocator::gbm::GbmConvertError;
    ...
    > From<PrepareFrameError<A, F, T>> for SwapBuffersError
{
    ...
    fn from(err: PrepareFrameError<A, F, T>) -> SwapBuffersError {
        match err {
            x @ PrepareFrameError::NoSupportedPlaneFormat
            | x @ PrepareFrameError::NoSupportedRendererFormat
            | x @ PrepareFrameError::PrimaryPlaneClaimFailed
            | x @ PrepareFrameError::NoFramebuffer => SwapBuffersError::ContextLost(Box::new(x)),
            x @ PrepareFrameError::NoFreeSlotsError | x @ PrepareFrameError::EmptyFrame => {
                SwapBuffersError::TemporaryFailure(Box::new(x))
            }
