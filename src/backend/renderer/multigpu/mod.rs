...
impl<A: GraphicsApi> GpuManager<A> {
    ...
    pub fn new<C>(api: A, capabilities: C) -> Result<GpuManager<A>, Error<A, A>>
    where
        C: Into<Option<Vec<Capability>>>,
    {
        ...
        api.enumerate(&mut devices, &capabilities)
    ...
    pub fn single_renderer<'api>(
        ...
        if !self.devices.iter().any(|dev| dev.node() == device) {
            self.api
                .enumerate(&mut self.devices, &self.capabilities)
                .map_err(Error::RenderApiError)?;
        }
        ...
        <B::Device as ApiDevice>::Renderer: ImportDma,
    {
            ...
            .any(|device| device.node() == render_device)
        {
            render_api
                .api
                .enumerate(&mut render_api.devices, &render_api.capabilities)
                .map_err(Error::RenderApiError)?;
        }
            ...
            .any(|device| device.node() == target_device)
        {
            target_api
                .api
                .enumerate(&mut target_api.devices, &target_api.capabilities)
