...
use crate::{
    backend::{
        allocator::{
            dmabuf::{AsDmabuf, WeakDmabuf},
            Allocator, Buffer as BufferTrait, Format, Fourcc,
        },
        drm::DrmNode,
        SwapBuffersError,
    },
...
#[derive(Debug)]
pub struct GpuManager<A: GraphicsApi, Alloc: Allocator> {
    api: A,
    devices: Vec<A::Device>,
    dma_source: HashMap<WeakDmabuf, DrmNode>,
    log: ::slog::Logger,
    _alloc_type: PhantomData<Alloc>,
}

/// Errors generated by [`GpuManager`] and [`MultiRenderer`].
#[derive(thiserror::Error)]
pub enum Error<R: GraphicsApi, T: GraphicsApi, A: Allocator>
where
    R::Error: 'static,
    T::Error: 'static,
    A::Error: 'static,
    <A as Allocator>::Buffer: AsDmabuf,
    <<A as Allocator>::Buffer as AsDmabuf>::Error: 'static,
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    ...
    /// The render allocator errored on buffer creation
    #[error("The render allocator failed creating a buffer {0:?}")]
    AllocatorError(#[source] A::Error),
    /// Exporting the allocated buffer failed
    #[error("Exporting the allocated buffer failed: {0:?}")]
    AsDmabufError(#[source] <<A as Allocator>::Buffer as AsDmabuf>::Error),
...
impl<R: GraphicsApi, T: GraphicsApi, A: Allocator> fmt::Debug for Error<R, T, A>
where
    R::Error: 'static,
    T::Error: 'static,
    A::Error: 'static,
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::AllocatorError(err) => write!(f, "Error::AllocatorError({:?})", err),
            Error::AsDmabufError(err) => write!(f, "Error::AsDmabufError({:?})", err),
...
impl<R: GraphicsApi + 'static, T: GraphicsApi + 'static, A: Allocator> From<Error<R, T, A>>
    for SwapBuffersError
where
    A: 'static,
    <A as Allocator>::Buffer: AsDmabuf,
    <<A as Allocator>::Buffer as AsDmabuf>::Error: Send + Sync,
...
{
    fn from(err: Error<R, T, A>) -> SwapBuffersError {
        match err {
            x @ Error::NoDevices
            | x @ Error::NoDevice(_)
            | x @ Error::DeviceMissing
            | x @ Error::AllocatorError(_) => SwapBuffersError::ContextLost(Box::new(x)),
            x @ Error::MismatchedDevice(_) | x @ Error::ImportFailed | x @ Error::AsDmabufError(_) => {
...
impl<A, Alloc> GpuManager<A, Alloc>
where
    A: GraphicsApi,
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
{
    ...
    ) -> Result<GpuManager<A, Alloc>, Error<A, A, Alloc>> {
    ...
    /// Create a [`MultiRenderer`] for a single device to avoid additional code paths.
    pub fn single_renderer<'api>(
        &'api mut self,
        device: &DrmNode,
    ) -> Result<MultiRenderer<'api, 'api, 'static, A, A, Alloc>, Error<A, A, Alloc>> {
        ...
        self.dma_source.retain(|weak, _| weak.upgrade().is_some());
    ...
    pub fn renderer<'api, 'alloc>(
        ...
        render_allocator: &'alloc mut Alloc,
        offscreen_format: Fourcc,
    ) -> Result<MultiRenderer<'api, 'api, 'alloc, A, A, Alloc>, Error<A, A, Alloc>> {
    ...
    ///   to find supported options and referr to the documentations of the used `GraphicsApi` for possible
    ///   (performance) implication of selecting a specific `Target`.
    pub fn cross_renderer<'render, 'target, 'alloc, B: GraphicsApi, OtherAlloc>(
        render_api: &'render mut Self,
        target_api: &'target mut GpuManager<B, OtherAlloc>,
        render_device: &DrmNode,
        target_device: &DrmNode,
        render_allocator: &'alloc mut Alloc,
        offscreen_format: Fourcc,
    ) -> Result<MultiRenderer<'render, 'target, 'alloc, A, B, Alloc>, Error<A, B, Alloc>>
    where
        OtherAlloc: Allocator,
        <OtherAlloc as Allocator>::Buffer: AsDmabuf,
    {
        if !render_api
            .devices
            .iter()
        ...
        render_api.dma_source.retain(|weak, _| weak.upgrade().is_some());
        ...
        match buffer_type(buffer) {
            Some(BufferType::Dma) => {
                ...
                if (matches!(dma_source, Entry::Vacant(_))
                    ...
                    && ImportDma::dmabuf_formats(target_device.renderer()).any(|f| f == &dmabuf.format())
                {
                    match target_device
                        .renderer_mut()
                            ...
                            .import_dma_buffer(buffer, Some(surface), damage)
                    {
                        ...
                        let mappings = if gpu_texture.get::<A>(&target).is_none() {
                            ...
                            damage
                                ...
                                .collect::<Result<Vec<_>, Error<A, A, Alloc>>>()?
                        };
...
struct TargetData<'target, 'alloc, T: GraphicsApi, Alloc> {
    device: &'target mut T::Device,
    render_allocator: &'alloc mut Alloc,
    cached_buffer: Option<Dmabuf>,
    format: Fourcc,
}

impl<'target, 'alloc, T: GraphicsApi, Alloc> fmt::Debug for TargetData<'target, 'alloc, T, Alloc> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TargetData")
            .field("device", &"...")
            .field("render_allocator", &"...")
...
#[derive(Debug)]
pub struct MultiRenderer<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> {
    dma_source: Option<&'render mut HashMap<WeakDmabuf, DrmNode>>,
    render: &'render mut R::Device,
    target: Option<TargetData<'target, 'alloc, T, Alloc>>,
    other_renderers: Vec<&'render mut R::Device>,
    log: ::slog::Logger,
}

impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc>
    AsRef<<R::Device as ApiDevice>::Renderer> for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
{
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc>
    AsMut<<R::Device as ApiDevice>::Renderer> for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
{
...
pub struct MultiFrame<'render, 'target, 'frame, R: GraphicsApi + 'frame, T: GraphicsApi, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    ...
    import_texture: Option<<<T::Device as ApiDevice>::Renderer as Renderer>::TextureId>,
    render: *mut &'render mut R::Device,
    target: Option<&'frame mut &'target mut T::Device>,

    dst_transform: Transform,
    size: Size<i32, Physical>,
    damage: Vec<Rectangle<i32, Physical>>,
    // We need this for the associated Error type of the Frame implementation
    _types: std::marker::PhantomData<Alloc>,
    log: ::slog::Logger,
}

impl<'render, 'target, 'frame, R: GraphicsApi + 'frame, T: GraphicsApi, Alloc> fmt::Debug
    for MultiFrame<'render, 'target, 'frame, R, T, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
    <Alloc as Allocator>::Error: 'static,
...
impl<'render, 'target, 'frame, R: GraphicsApi, T: GraphicsApi, Alloc>
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> Unbind
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc, Target> Offscreen<Target>
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc, Target> Bind<Target>
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
    <T::Device as ApiDevice>::Renderer: Bind<Target>,
    <R::Device as ApiDevice>::Renderer: Bind<Target>,
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> Renderer
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    type Error = Error<R, T, Alloc>;
    type TextureId = MultiTexture;
    type Frame<'frame> = MultiFrame<'render, 'target, 'frame, R, T, Alloc> where Self: 'frame;
    ...
    fn render<'frame>(
        ...
        size: Size<i32, Physical>,
        dst_transform: Transform,
    ) -> Result<MultiFrame<'render, 'target, 'frame, R, T, Alloc>, Self::Error> {
        let import_texture = if let Some(target) = self.target.as_mut() {
            // make sure we have a dmabuf
            let buffer_size = size.to_logical(1).to_buffer(1, dst_transform);
            if target
                .cached_buffer
                .as_ref()
                .filter(|dmabuf| dmabuf.size() == buffer_size)
                .is_none()
            {
                ...
                let dmabuf = render_buffer.export().map_err(Error::AsDmabufError)?;
                target.cached_buffer = Some(dmabuf);
            }

            let dmabuf = target.cached_buffer.clone().unwrap();
            ...
            slog::debug!(self.log, "Successfully bound offscreen buffer to renderer");

            {
                let render = self.render.renderer_mut();
                let size = dmabuf.size().to_logical(1, Transform::Normal).to_physical(1);
                let mut frame = render.render(size, Transform::Normal).map_err(Error::Render)?;
                frame
                    .clear(
                        [0.0, 0.0, 0.0, 1.0],
            ...
            // try to import on target
            match target.device.renderer_mut().import_dmabuf(
                &dmabuf,
                Some(&[Rectangle::from_loc_and_size((0, 0), buffer_size)]),
            ) {
                Ok(texture) => {
                    // import successful!
                    slog::debug!(self.log, "Target device dmabuf import successful!");
                    Some(texture)
                }
                Err(err) => {
        ...
        Ok(MultiFrame {
            ...
            target: self.target.as_mut().map(|t| &mut t.device),
            dst_transform,
            size,
            damage: Vec::new(),
            _types: PhantomData::<Alloc>,
            log,
        })
    }
}
...
impl<'render, 'target, 'frame, R: GraphicsApi, T: GraphicsApi, Alloc>
    MultiFrame<'render, 'target, 'frame, R, T, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
    <Alloc as Allocator>::Error: 'static,
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    fn finish_internal(&mut self) -> Result<(), Error<R, T, Alloc>> {
        if let Some(frame) = self.frame.take() {
            ...
            if let Some(target) = self.target.as_mut() {
                if let Some(texture) = self.import_texture.take() {
                    // try gpu copy
                    let damage = damage
                        .iter()
                        .map(|rect| {
                    ...
                    for (mapping, rect) in mappings {
                        let slice = render
                            .renderer_mut()
                            .map_texture(&mapping)
                            .map_err(Error::Render::<R, T, Alloc>)?;
                        let texture = target
                            .renderer_mut()
                        ...
                        for damage_rect in damage.iter().filter_map(|dmg_rect| dmg_rect.intersection(rect)) {
                            let dst = damage_rect
                                ...
                                .to_physical(1);
                            let src =
                                Rectangle::from_loc_and_size(damage_rect.loc - rect.loc, damage_rect.size)
                                    .to_f64();
...
impl<'render, 'target, 'frame, R: GraphicsApi, T: GraphicsApi, Alloc> Drop
    for MultiFrame<'render, 'target, 'frame, R, T, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
...
impl<'render, 'target, 'frame, R: GraphicsApi, T: GraphicsApi, Alloc> Frame
    for MultiFrame<'render, 'target, 'frame, R, T, Alloc>
where
    Alloc: Allocator,
    <Alloc as Allocator>::Buffer: AsDmabuf,
...
#[cfg(feature = "wayland_frontend")]
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ImportMemWl
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ImportMem
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
#[cfg(feature = "wayland_frontend")]
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ImportDmaWl
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ImportDma
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc>
    MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
    <R::Device as ApiDevice>::Renderer: ImportDma + ImportMem + ExportMem,
    <T::Device as ApiDevice>::Renderer: ExportMem,
    <<R::Device as ApiDevice>::Renderer as ExportMem>::TextureMapping: 'static,
        ...
        if let Some(import_renderer) = self.target.as_mut().filter(|target| match source {
                ...
                .import_dmabuf(dmabuf, Some(&*new_damage))
            {
                ...
                let mappings = new_damage
                    ...
                    .collect::<Result<Vec<_>, Error<R, T, Alloc>>>()?;
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ExportMem
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
    <T::Device as ApiDevice>::Renderer: ExportMem,
...
impl<'render, 'target, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc> ExportDma
    for MultiRenderer<'render, 'target, 'alloc, R, T, Alloc>
where
    <R::Device as ApiDevice>::Renderer: ExportDma,
...
impl<'a, 'b, 'alloc, R: GraphicsApi, T: GraphicsApi, Alloc, Target> Blit<Target>
    for MultiRenderer<'a, 'b, 'alloc, R, T, Alloc>
where
    <R::Device as ApiDevice>::Renderer: Blit<Target>,
    <T::Device as ApiDevice>::Renderer: Blit<Target>,
