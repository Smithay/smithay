...
type SeatObjectId = ObjectId;
type NotificationObjectId = ObjectId;

type SeatMap<T> = HashMap<SeatObjectId, T>;
type NotificationMap<T> = HashMap<NotificationObjectId, T>;

/// State of ext-idle-notify module
#[derive(Debug)]
pub struct IdleNotifierState<D> {
    global: GlobalId,
    notifications: SeatMap<NotificationMap<ExtIdleNotificationV1>>,
