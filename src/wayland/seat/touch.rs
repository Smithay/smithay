...
use crate::{
    ...
    utils::{Logical, Point},
    wayland::seat::wl_surface::WlSurface,
};
...
impl TouchHandle {
    ...
    pub fn up(&self, dh: &mut DisplayHandle<'_>, serial: u32, time: u32, slot: TouchSlot) {
...
impl TouchInternal {
    ...
    fn down(
        ...
        self.with_focused_handles(dh, slot, |dh, handle| {
            handle.down(dh, serial, time, surface, slot.into(), x, y)
        });
    }

    fn up(&self, dh: &mut DisplayHandle<'_>, serial: u32, time: u32, slot: TouchSlot) {
        self.with_focused_handles(dh, slot, |dh, handle| handle.up(dh, serial, time, slot.into()));
    }
