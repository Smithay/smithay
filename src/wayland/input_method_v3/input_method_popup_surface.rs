...
impl PopupSurface {
    ...
    pub fn positioner(&self) -> PositionerState {
        ...
        role_data.positioner.lock().unwrap().clone()
    }
    ...
    pub fn send_pending_configure(&mut self) {
        ...
        if new_state != sent_state {
            ...
            match (new_state.repositioned, sent_state.repositioned) {
                (Some(new), sent) => {
                    if Some(new) != sent {
        ...
        match request {
            ...
            Request::Reposition { positioner, token } => {
                ...
                let popup = {
                    ...
                    let positioner = positioner.inner.lock().unwrap().clone();
                    ...
                    let cursor = instance.cursor_rectangle.clone();
                    let popup = instance
                        .popup_handles
                        .iter_mut()
