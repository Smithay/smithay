...
impl DrmTimelineInner {
    pub(super) fn update_device(&self, device: &DrmDeviceFd) -> io::Result<()> {
        *self.dev_ctx.lock().unwrap() = DrmTimelineDeviceSpecific::import(self.timeline_fd.as_fd(), device)?;
        Ok(())
    }
...
impl DrmTimelineDeviceSpecific {
    ...
    fn invalidate(&mut self) {
        ...
        // trigger event fds
        for eventfd in self.event_fds.drain(..).filter_map(|x| Weak::upgrade(&x)) {
...
impl Drop for DrmTimelineDeviceSpecific {
    fn drop(&mut self) {
        ...
        // trigger event fds
        for eventfd in self.event_fds.iter().filter_map(Weak::upgrade) {
            let _ = rustix::io::write(&eventfd, &[1]);
        }
    }
}
...
impl DrmSyncPoint {
    ...
    pub fn eventfd(&self) -> io::Result<Arc<OwnedFd>> {
        ...
        let fd = Arc::new(fd);
        ctx.event_fds.retain(|fd| fd.upgrade().is_some());
        ctx.event_fds.push(Arc::downgrade(&fd));
        Ok(fd)
    }
