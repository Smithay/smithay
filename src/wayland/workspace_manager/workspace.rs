...
impl Workspace {
    ...
    fn builder(name: String) -> Builder {
        Builder {
            id: None,
            name,
...
impl WorkspaceManagerState {
    ...
    where
        ...
        D: Dispatch<ExtWorkspaceHandleV1, WorkspaceData>,
    {
        let mut handle = builder.build(self.instances.len(), self.done_needed.clone());
...
impl Drop for Inner {
    fn drop(&mut self) {
        // TODO: Remove from current group if any.
