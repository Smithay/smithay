    ...
    ($state:ty,
     ...
     $(#[$cached_state_meta:meta])* $cached_state_name:ident) => {
        ...
        pub struct $attributes_name {
                ...
                /// by the client along with its serial. This state should be cloned to the current
                /// during a commit. Note that this state can be newer than the last acked state at
                /// the time of the last commit.
                pub last_acked: Option<(Serial, $state)>,
        ...
        impl $attributes_name {
            fn ack_configure(&mut self, serial: Serial) -> Option<Configure> {
                ...
                self.last_acked = Some((serial, configure.state.clone()));
            ...
            pub fn current_server_state(&self) -> $state {
                ...
                self.pending_configures
                    .last()
                    .map(|c| &c.state)
                    .or_else(|| self.last_acked.as_ref().map(|(_, state)| state))
                    .cloned()
                    .unwrap_or_default()
            }
        ...
        pub struct $cached_state_name {
            /// Serial and state last acknowledged by the client at the time of the commit.
            ///
...
impl ToplevelSurface {
    ...
    pub(crate) fn pre_commit_hook<D: XdgShellHandler + 'static>(
        ...
        let error = compositor::with_states(surface, |states| {
            ...
            let has_buffer = match &guard_surface.pending().buffer {
                ...
                None => pending.last_acked.is_some(),
            };
            let got_unmapped = matches!(guard_surface.pending().buffer, Some(BufferAssignment::Removed));
        ...
        F: FnOnce(Option<&ToplevelState>) -> T,
    {
        compositor::with_states(&self.wl_surface, |states| {
            ...
            let current = guard.current().last_acked.as_ref().map(|(_serial, state)| state);
            f(current)
        })
    }
    ...
    pub fn send_configure(&self) -> Result<Serial, PopupConfigureError> {
        ...
        compositor::with_states(&self.wl_surface, |states| {
            ...
            let is_reactive = cached
                ...
                .last_acked
                .as_ref()
                .is_some_and(|(_, state)| state.positioner.reactive);
