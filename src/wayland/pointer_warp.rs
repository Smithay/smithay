...
//! events to it with [`delegate_pointer_warp`].
//!
//! ```
...
pub mod server {
    ...
    pub mod __interfaces {
        ...
        wayland_scanner::generate_interfaces!("pointer-warp-v1.xml");
    }
    use self::__interfaces::*;

    wayland_scanner::generate_server_code!("pointer-warp-v1.xml");
}

use server::wp_pointer_warp_v1::{self, WpPointerWarpV1};
use wayland_server::{
    backend::GlobalId,
...
macro_rules! delegate_pointer_warp {
    ($(@<$( $lt:tt $( : $clt:tt $(+ $dlt:tt )* )? ),+>)? $ty: ty) => {
        $crate::reexports::wayland_server::delegate_global_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            $crate::wayland::pointer_warp::server::wp_pointer_warp_v1::WpPointerWarpV1: ()
