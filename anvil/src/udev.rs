...
use smithay::{
    backend::{
        ...
        renderer::{
            damage::Error as OutputDamageTrackerError, element::{memory::MemoryRenderBuffer, AsRenderElements}, gles::GlesRenderer, multigpu::{gbm::GbmGlesBackend, GpuManager, MultiRenderer}, Color32F, DebugFlags, ImportDma, ImportMemWl
        },
        session::{
...
impl AnvilState<UdevData> {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            let drm_output = match device
                .drm_output_manager
                .initialize_output::<_, OutputRenderElements<UdevRenderer<'_>, WindowRenderElement<UdevRenderer<'_>>>, _>(
                    &mut renderer,
                    crtc,
                    drm_mode,
                    &[connector.handle()],
    ...
    fn connector_disconnected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        let mut renderer = self
                    ...
                    .single_renderer(&device.render_node)
                    .unwrap();
        let _ = device.drm_output_manager.reset_format::<_, OutputRenderElements<UdevRenderer<'_>, WindowRenderElement<UdevRenderer<'_>>>, _>(&mut renderer, |_| { 
            // FIXME: For a flicker free operation we should return the actual elements for this output..
            // Instead we just use black to "simulate" a modeset :)
            (&[], Color32F::BLACK) 
        });
