...
impl AnvilState<UdevData> {
    ...
    fn on_pointer_move<B: InputBackend>(&mut self, _dh: &DisplayHandle, evt: B::PointerMotionEvent) {
        ...
        if let Some((surface, surface_loc)) = &under {
            if let Some(surface) = surface.wl_surface().as_ref() {
                let pointer_confined = with_pointer_constraint(surface, &pointer, |constraint| {
                    ...
                    match &*constraint {
                        PointerConstraint::Confined(confined) => confined.region().cloned(),
                        _ => None,
                    }
                });

                if let Some(region) = pointer_confined {
                    use smithay::wayland::pointer_constraints::PointerConstraintShape;
                    // TODO: Shape build per motion might be slow
                    let shape = PointerConstraintShape::from_region(&region);
                    pointer_location = shape.clamp_point(pointer_location - *surface_loc) + *surface_loc;
                }
            }
        }
